...
function $checkTimes(times) {
	if (!adIsNumber(times, { finite: true, integer: true, minimum: 1, safe: true })) {
		throw new TypeError(`Argument \`times\` must be type of number (finite, integer, and safe) and greater than 0!`);
	};
};
/**
 * @private
...
class SymmetricCryptor {
	...
	constructor(passphrase) {
		...
		this.#passphraseStorage = cryptoCreateHash("sha256").update(passphrase).digest().slice(0, 32);
	};
	/**
	 * @method decrypt
