...
function $decrypt(data, hash) {
	...
	let decipher = crypto.createDecipheriv("AES-256-CBC", hash, encrypted.subarray(0, 16));
	...
	return decrypted.substring(0, decryptedLength - decrypted.charCodeAt(decryptedLength - 1));
};
/**
 * @private
 * @function $encrypt
...
class SymmetricCryptor {
	...
	constructor(passphrase) {
		...
		this.#passphraseStorage = crypto.createHash("sha256").update(passphrase).digest().subarray(0, 32);
	};
	/**
	 * @method decrypt
